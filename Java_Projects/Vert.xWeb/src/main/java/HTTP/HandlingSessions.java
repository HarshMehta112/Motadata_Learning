...
public class HandlingSessions extends AbstractVerticle
{
    ...
    public void start (Promise< Void > startPromise) throws Exception
    {
        ...
        SessionHandler sessionHandler = SessionHandler.create(LocalSessionStore
                .create(vertx, "myapp.session", 10000)).setSessionTimeout(10000);

        router.get("/session/start").handler(sessionHandler).handler(context ->
        {
            Session session = context.session();

            session.put("name", "harsh");
    ...
    public static void main (String[] args)
    {
        ...
        vertx.deployVerticle(HandlingSessions.class.getName());
    }
